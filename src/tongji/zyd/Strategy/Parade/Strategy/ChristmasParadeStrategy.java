package tongji.zyd.Strategy.Parade.Strategy;

import tongji.zyd.Strategy.Parade.Employee.Employee;

/*
 * 圣诞巡游策略，继承自ParadeStrategy
 * 由于该策略仅用于提供于巡游相关的信息，使用过程中自身内容不会发生改变，故使用单例模式
 */
public class ChristmasParadeStrategy extends ParadeStrategy {
    /*
    ...
    static public ParadeStrategy getInstance(){
        if(instance==null){
            synchronized (ChristmasParadeStrategy.class){
                if(instance==null){
                    instance=new ChristmasParadeStrategy();
                }
            }
        }
     ...
     * 单例模式构造函数为private
     */
    private ChristmasParadeStrategy(){
